    ...
    <div class="listBoxContentGen">
        <form id="form">
            <table width="100%" border="0" align="center" cellpadding="3" cellspacing="0" class="CommonTb">
                <tbody>
				<input id="id" name="id" type="hidden" value="${userTagProPage.id }">
                <tr>
					<td align="right">
					...
					<td class="value">
							<select id="tagId" name="tagId" datatype="*">
								<c:forEach items="${tagList}" var="tag">
								    
									<option value="${tag.id }" <c:if test="${tag.id eq '${userTagProPage.tagId}' }">selected="selected"</c:if>>${tag.name}</option>
								</c:forEach>
							</select>
                ...
                <tr>
                    <td align="center"><input type="button" name="Button1" value="提 交" id="Button2"
                                                          class="btn btn-primary" onclick="dosubmit()"><span
                            id="showText"></span></td>
					 <td align="center"><input type="button" name="Button1" value="删  除" id="Button2"
                                                          class="btn btn-primary" onclick="dodelete()"><span
                            id="showText"></span></td>
                </tr>
                </tbody>
            </table>
...
function dosubmit() {
        $.ajax({
            cache: true,
            type: "POST",
            url: "userTagProController.do?doUpdate",
	...
	function dodelete() {
        $.ajax({
			...
			data: {tagId:${userTagProPage.tagId},productId:${userTagProPage.productId}},
			async: false,
			dataType: 'json',
			error: function (request) {
				showContent('', 'frontController.do?list');
	...
	$.ajax({
		...
		success: function (data) {
			data = eval(data);
			$('#tagId').html("");
			var tagId = ${userTagProPage.tagId };
			var str = "<option value=\"\">---请选择---</option>";
			for (var i = data.length-1, o; i >= 0; i--) {
				o = data[i];
				if(o.id==tagId){
					str += "<option value=\""+o.id+"\" selected=\"seleted\">"+o.name+"</option>";
				}else{
