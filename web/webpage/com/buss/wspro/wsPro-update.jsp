...
<html>
 ...
 <body>
  <t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="wsProController.do?doUpdate" tiptype="1">
						...
						<td class="value">
							<select id="wid" name="wid" datatype="*">
								<c:forEach items="${wsList}" var="ws">
									<option value="${ws.id }" ${ws.id==wsProPage.wid?'selected':''}>${ws.workName}</option>
								</c:forEach>
							</select>
							<%--<t:dictSelect dictTable="m_workspace" dictText="w_name" dictField="id" field="wid" hasLabel="false"--%>
										  <%--defaultVal="${wsProPage.wid}"></t:dictSelect>--%>
