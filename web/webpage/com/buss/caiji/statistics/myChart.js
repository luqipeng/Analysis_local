...
var MyHighcharts = function(options){
	...
	this.draw = function(i,reportType,categoryId,productId){
		// 显示等待信息
		$("#"+reportType+"Container").empty();
		$("#"+reportType+"Container").append("<p style=\"text-align: center\"><img src=\"/images/loading.gif\" alt=\"加载中，请稍候...\" /></p>");
		this.initLocalData(i,options);
		$.post("productController.do?getLine&reportType="+reportType+"&categoryId="+categoryId+"&productId="+productId,this.initParams(tempChart),this.callBackChart);
	}
	...
	this.callBackChart = function(data){
		...
		if(tempChart.timetype != 0){
			tempChart.categoriesLocal = data.categoriesLocal;
		}
		for(var i=0;i<data.series.length;i++){
			var series = data.series[i];
			switch (series.typechart) {
				case "0"://线状图
	...
	this.callBackLine = function(data){
		...
		var yTitle = "";
		switch(data.type){
			case "all":title = "产品价格统计";
						yTitle = "价格";
			break;
			case "index":title = "产品排名统计";
						yTitle = "排名";
			break;
			case "line":title = "产品销量统计";
						yTitle = "销量";
			break;
		}
		if(data.series.length>2){
			new Highcharts.Chart({
				...
				yAxis: [{
					...
					opposite: true,
					tickPositions: [0, 20, 40, 60, 80, 100, 150, 200]
	...
	this.pmt = function(str,i){//时间格式化		
		...
		if(str.length==10){
			...
			str = str.substring(0,2)+"月"+str.substring(3,5)+"号";
			//str = str + "年";
		}
		return str;
	}
