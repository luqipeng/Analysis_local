...
conf=$.extend(true,{},$.tools.overlay.conf,conf);this.each(function(){el=new Overlay($(this),conf);instances.push(el);$(this).data("overlay",el);});return conf.api?el:this;};})(jQuery);;(function($){"use strict";var feature={};feature.fileapi=$("<input type='file'/>").get(0).files!==undefined;feature.formdata=window.FormData!==undefined;$.fn.ajaxSubmit=function(options){if(!this.length){log('ajaxSubmit: skipping submit process - no element selected');return this;}
...
else if(window.opera&&window.opera.postError){window.opera.postError(msg);}}})(jQuery);(function($,window,undefined){'$:nomunge';var elems=$([]),jq_resize=$.resize=$.extend($.resize,{}),timeout_id,str_setTimeout='setTimeout',str_resize='resize',str_data=str_resize+'-special-event',str_delay='delay',str_throttle='throttleWindow';jq_resize[str_delay]=250;jq_resize[str_throttle]=true;$.event.special[str_resize]={setup:function(){if(!jq_resize[str_throttle]&&this[str_setTimeout]){return false;}
var elem=$(this);elems=elems.add(elem);$.data(this,str_data,{w:elem.width(),h:elem.height()});if(elems.length===1){loopy();}},teardown:function(){if(!jq_resize[str_throttle]&&this[str_setTimeout]){return false;}
var elem=$(this);elems=elems.not(elem);elem.removeData(str_data);if(!elems.length){clearTimeout(timeout_id);}},add:function(handleObj){if(!jq_resize[str_throttle]&&this[str_setTimeout]){return false;}
var old_handler;function new_handler(e,w,h){var elem=$(this),data=$.data(this,str_data);data.w=w!==undefined?w:elem.width();data.h=h!==undefined?h:elem.height();old_handler.apply(this,arguments);};if($.isFunction(handleObj)){old_handler=handleObj;return new_handler;}else{old_handler=handleObj.handler;handleObj.handler=new_handler;}}};function loopy(){timeout_id=window[str_setTimeout](function(){elems.each(function(){var elem=$(this),width=elem.width(),height=elem.height(),data=$.data(this,str_data);if(width!==data.w||height!==data.h){elem.trigger(str_resize,[data.w=width,data.h=height]);}});loopy();},jq_resize[str_delay]);};})(jQuery,this);jQuery(function($){$('.editable-form').submit(function(){var form=$(this);var loading=$('#editable-loading');var showError=function(msg){if(msg){msg=': '+msg;}else{msg='';}
alert('An error occurred'+msg);loading.hide();form.show();};loading.css({top:($(window).height()-loading.height())/2+$(window).scrollTop()+"px",left:($(window).width()-loading.width())/2+$(window).scrollLeft()+"px"});form.hide();loading.show();if(typeof tinyMCE!="undefined"){tinyMCE.triggerSave();}
form.ajaxSubmit({success:function(data){if(data&&data!='<head></head><body></body>'){showError(data);}else{location.reload();}},error:function(){showError();}});return false;})
var realign=function(){$.each($('.editable-original'),function(i,editable){editable=$(editable);var link=editable.next('.editable-link');link.offset({top:editable.offset().top,left:editable.offset().left-link.outerWidth()-1});var expose={color:'#333',loadSpeed:200,opacity:0.9};var overlay={expose:expose,closeOnClick:true,close:':button',left:'center',top:'center'};link.overlay(overlay);var highlight=link.next('.editable-highlight')
highlight.css({width:editable.width(),height:editable.height()});highlight.offset({top:editable.offset().top,left:editable.offset().left});});};realign();$('.editable-link').hover(function(e){$(this).next('.editable-highlight').css('visibility','visible');},function(e){$(this).next('.editable-highlight').css('visibility','hidden');});$('body, .editable-original').on('resize',function(e){realign();});var cookie='mezzanine-admin-toolbar';var at=('; '+document.cookie).indexOf('; '+cookie+'=');var closed=false;if(at>-1){closed=document.cookie.substr(at+
cookie.length+1).split(';')[0];}
$(window.__toolbar_html).appendTo('body');$('#editable-toolbar-toggle').click(function(){var toggle=$(this);var links=$('.editable-link');var toolbar=$('#editable-toolbar *[id!=editable-toolbar-toggle]');if(toggle.text()=='<<'){toggle.text('>>');toolbar.hide();links.css('visibility','hidden');document.cookie=cookie+'=1; path=/';}else{toggle.text('<<');toolbar.show();links.css('visibility','visible');document.cookie=cookie+'=; path=/';}
return false;});$('#editable-toolbar-logout').click(function(){$('#editable-toolbar').submit();return false;});$('#editable-toolbar').fadeIn('slow');if(!closed){$('#editable-toolbar-toggle').click();}});