 ...
 */
@Controller
@RequestMapping("/reportProductController")
public class ReportProductController extends BaseController {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(ReportProductController.class);
    //班级学生人数比例分析国际化Key
	 ...
	 * @return
	 */
	@RequestMapping(params = "productStatisticTabs")
	public ModelAndView productStatisticTabs(HttpServletRequest request) {
		...
		if(request.getParameter("productId")==null){
			request.setAttribute("productId", request.getParameter("productIds"));
		}
		
		String productId = request.getParameter("productId");
		if(productId.split(";").length<2){
			 List<ProductEntity> productEntities = productService.findByProperty(ProductEntity.class, "productId", productId);
			 List<String> categoryList = new ArrayList<String>();
			 for(ProductEntity productEntity:productEntities){
				 categoryList.add(productEntity.getCategoryId()); 
			 }
			 request.setAttribute("categoryIds", categoryList);
			 if(categoryList.size()>0){
				 request.setAttribute("categoryId", categoryList.get(0));
			 }
			 
			 if(productEntities.size()>0){
				 request.setAttribute("product", productEntities.get(0));
			 }
		}
		
//		return new ModelAndView("report/productStatistic");
